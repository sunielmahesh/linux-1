...
static int sn65dsi_attach(struct drm_bridge *bridge, enum drm_bridge_attach_flags flags)
{
	...
	sn->dsi->mode_flags = MIPI_DSI_MODE_VIDEO | MIPI_DSI_MODE_VIDEO_SYNC_PULSE;
...
static int sn65dsi_parse_dt(struct sn65dsi *sn)
{
	...
	if (ret < 0) {
		dev_info(dev,
			 "failed to find panel_or_bridge (ret = %d)\n", ret);
		return ret;
	}
	if (!panel)
		return -ENODEV;
	...
	if (sn->dsi_lanes < 1 || sn->dsi_lanes > 4) {
		DRM_DEV_ERROR(dev, "invalid data lanes %d\n", sn->dsi_lanes);
		return -EINVAL;
	}
	...
	dev_info(dev, "%s lanes = %d done!\n", __func__, sn->dsi_lanes);
...
static int sn65dsi_probe(struct i2c_client *client)
{
	...
	if (ret) {
		dev_err(&client->dev, "dts parsing failed (ret=%d)\n", ret);
		return ret;
	}
